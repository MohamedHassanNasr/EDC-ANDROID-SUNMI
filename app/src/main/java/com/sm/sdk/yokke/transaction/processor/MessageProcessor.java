...
public class MessageProcessor {
    ...
    private static void getGenerateQris(HashMap<String, byte[]> map, TransData transData) {
        ...
        String getResponse = Tools.hexToString(Tools.bcd2Str(map.get("39")));
        if ("00".equals(getResponse)){
    ...
    private static void parseAndSaveDataFromBit44(HashMap<String, byte[]> map, TransData transData) {
        ...
        promotionID, memberbankTID, memberbankMID;
        ...
        if (bit44.length()>16){
            promotionID = Tools.hexToAscii(sb.substring(16, 28));
            memberbankTID = Tools.hexToAscii(sb.substring(28, 44));
            memberbankMID = Tools.hexToAscii(sb.substring(44, 74));
        }
        else{
            promotionID = null;
            memberbankTID = null;
        ...
        if(TransConstant.TRANS_TYPE_INQUIRY_QRIS.equals(transData.getTransactionType())
                || TransConstant.TRANS_TYPE_GENERATE_QRIS.equals(transData.getTransactionType())
                || TransConstant.TRANS_TYPE_ANY_TRANS_QRIS.equals(transData.getTransactionType())
                || TransConstant.TRANS_TYPE_REFUND_QRIS.equals(transData.getTransactionType())) {
            cardTypeBit44 = null;
    ...
    private static void getQRIS(HashMap<String, byte[]> map, TransData transData) {
        ...
        if (map.containsKey("4")) {
            String amount = Tools.hexToString(Tools.bcd2Str(map.get("4")));
            StringBuilder sb = new StringBuilder(amount);
            amount = sb.substring(0,amount.length()-2);
